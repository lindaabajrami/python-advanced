...
st.sidebar.header('Add New Books Data')
with st.sidebar.form('Book_form'):
    ...
    new_price = st.number_input("Price", min_value=0, max_value=1)
    new_year = st.number_input("Year", min_value=2009, max_value=2022, step=1)
    new_genre = st.selectbox("Genre", books_df['Genre'].unique())
    submit_button = st.form_submit_button(label="Add Books")
...
col2.metric('Uniques Titles', unique_titles)
